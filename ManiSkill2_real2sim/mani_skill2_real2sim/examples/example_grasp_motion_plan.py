...
python mani_skill2_real2sim/examples/example_grasp_motion_plan.py -e GraspMultipleSameObjectsInScene-v0 \
    -c arm_pd_ee_delta_pose_align_interpolate_by_planner_gripper_pd_joint_target_delta_pos_interpolate_by_planner -o rgbd \
    --enable-sapien-viewer  robot google_robot_static
...
def main():
    ...
    idx = 0
    # dh = -0.008 for google robot, 0.005 for others
    planner = GraspMotionPlanner(
        ee_action_scale=ee_action_scale,
        ee_rot_action_scale=ee_rot_action_scale,
        dh=-0.008 if is_google_robot else 0.005,
        is_google_robot=is_google_robot,
    )
    ...
    while True:
        ...
        ee_action, gripper_action = planner.plan_motion(object_position, qpos, tcp_pose)

        EE_ROT_ACTION = ee_rot_action_scale
        if env.viewer.window.key_press("1"):
        ...
        # if key == "v":
        #     visualize_observation(obs, env, opencv_viewer)
        ...
        # links = env.agent.robot.get_links()
        # for idx, link in enumerate(links):
        #     print(f"Link {link.get_name()} pose: {link.get_pose()}")
