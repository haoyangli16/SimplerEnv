...
from mani_skill2_real2sim.agents.robots.widowx import (
    WidowX,
    WidowXBridgeDatasetCameraSetup,
    WidowXSinkCameraSetup,
)
...
class CustomSceneEnv(BaseEnv):
        ...
        **kwargs,
        ...
        if rgb_overlay_path is not None:
            ...
            self.rgb_overlay_img = (
                cv2.cvtColor(cv2.imread(rgb_overlay_path), cv2.COLOR_BGR2RGB) / 255
            )  # (H, W, 3); float32
        else:
        ...
        assert ("background" in self.rgb_overlay_mode) or (
            "debug" in self.rgb_overlay_mode
        ), "Invalid rgb_overlay_mode"
    ...
    def _load_arena_helper(self, add_collision=True):
        ...
        if self.scene_name is None:
            if "google_robot_static" in self.robot_uid:
                scene_path = str(
                    self.scene_root / "stages/google_pick_coke_can_1_v4.glb"
                )  # hardcoded for now
            elif "widowx" in self.robot_uid:
                scene_path = str(
                    self.scene_root / "stages/bridge_table_1_v1.glb"
                )  # hardcoded for now
            else:
        ...
        if self.scene_name is not None:
            ...
            if "modern_bedroom" in self.scene_name:
                ...
                scene_pose = (
                    sapien.Pose([0.178, -2.235, 1.669], [0.007, 0, 0, -1]) * scene_pose
                )
            elif "modern_office" in self.scene_name:
                scene_offset = np.array([-1.6616, -3.0337, 0.0])
                scene_pose = (
                    sapien.Pose([-0.192, -1.728, 1.48], [0.709, 0, 0, -0.705])
                    * scene_pose
                )
        ...
        if (self.scene_name is None) or ("dummy" not in self.scene_name):
            ...
            if self.scene_name == "dummy":
                ...
                builder.add_box_visual(
                    half_size=np.array([10.0, 10.0, 0.017]), color=[1, 1, 1]
                )
                ...
                builder.add_box_visual(
                    pose=_pose, half_size=_half_size, material=rend_mtl
                )
                if add_collision:
                ...
                builder.add_box_visual(
                    half_size=np.array([10.0, 10.0, 0.017]), color=_color
                )
            else:
    ...
    def _initialize_agent(self):
        ...
        if "google_robot_static" in self.robot_uid:
            ...
            robot_init_height = (
                0.06205 + 0.017
            )  # base height + ground offset in default scene
    ...
    def _register_cameras(self):
        ...
        # camera = add_camera(
        #     name="", width=3770, height=2096, fovy=1.05, near=0.1, far=100
        # )
        # camera.set_local_pose(Pose([-0.404714, 0.235361, 1.62074], [0.73389, -0.00362903, 0.679247, 0.00392097]))
        pose = Pose(
            [-0.404714, 0.235361, 1.35], [0.73389, -0.00362903, 0.679247, 0.00392097]
        )
        # pose = look_at([0.3, 0, 0.6], [-0.1, 0, 0.1])
        return CameraConfig(
            "base_camera", pose.p, pose.q, 640, 480, np.pi / 2, 0.01, 10
        )
    ...
    def get_obs(self):
        ...
        if self._obs_mode == "image" and self.rgb_overlay_img is not None:
            ...
            for camera_name in self.rgb_overlay_cameras:
                ...
                assert (
                    "Segmentation" in obs["image"][camera_name].keys()
                ...
                seg = obs[
                    ...
                    "Segmentation"
                ]  # (H, W, 4); [..., 0] is mesh-level; [..., 1] is actor-level; [..., 2:] is zero (unused)
                actor_seg = seg[..., 1]
                ...
                rgb_overlay_img = cv2.resize(
                    self.rgb_overlay_img,
                    (
                        obs["image"][camera_name]["Color"].shape[1],
                        obs["image"][camera_name]["Color"].shape[0],
                    ),
                )
                if "debug" not in self.rgb_overlay_mode:
                    obs["image"][camera_name]["Color"][..., :3] = (
                        obs["image"][camera_name]["Color"][..., :3] * (1 - mask)
                        + rgb_overlay_img * mask
                    )
                else:
                    # debug
                    # obs['image'][camera_name]['Color'][..., :3] = obs['image'][camera_name]['Color'][..., :3] * (1 - mask) + rgb_overlay_img * mask
                    obs["image"][camera_name]["Color"][..., :3] = (
                        obs["image"][camera_name]["Color"][..., :3] * 0.5
                        + rgb_overlay_img * 0.5
                    )
    ...
    def advance_to_next_subtask(self):
        raise NotImplementedError(
            "advance_to_next_subtask is not implemented for this environment."
        )
